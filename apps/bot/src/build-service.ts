...
import { db, deployments, snapshots, buildArtifacts } from '@tscircuit-deploy/shared/db';
import { eq } from 'drizzle-orm';
import { generateDeploymentId, generatePreviewUrl, formatBuildTime } from '@tscircuit-deploy/shared/utils';
...
export class BuildService {
  ...
  async startBuild(context: BuildContext): Promise<string> {
    ...
    try {
      ...
      if (result.success) {
        ...
        await this.handleBuildFailure(deploymentId, result.errorMessage || 'Unknown build error');
      }
      
      return deploymentId;
    } catch (error) {
      console.error(`Build failed for deployment ${deploymentId}:`, error);
      await this.handleBuildFailure(deploymentId, error instanceof Error ? error.message : 'Build process failed');
      throw error;
    } finally {
  ...
  private async processBuild(deploymentId: string, context: BuildContext): Promise<DeploymentResult> {
    ...
    try {
      ...
      for (const circuitFile of context.circuitFiles) {
        ...
        const snapshot = await this.buildCircuitSnapshot(deploymentId, circuitFile);
        artifacts.push(...(await this.generateArtifacts(deploymentId, circuitFile, snapshot)));
      }
      ...
      return {
        success: false,
        deploymentId,
        errorMessage: error instanceof Error ? error.message : 'Build processing failed',
  ...
  private async buildCircuitSnapshot(deploymentId: string, circuitFile: { path: string; content: string; sha: string }) {
    try {
  ...
  private async executeCircuitBuild(circuitContent: string): Promise<Record<string, any>> {
  ...
  private async generateArtifacts(deploymentId: string, circuitFile: any, snapshot: any) {
    const artifacts = [];
    ...
    for (const type of artifactTypes) {
      try {
        ...
        const artifactData = await this.generateArtifactData(type, snapshot.data);
  ...
  private async generateArtifactData(type: string, snapshotData: any) {
    switch (type) {
      ...
      case 'gerber':
        return {
          type: 'gerber',
          files: ['copper_top.gbr', 'copper_bottom.gbr', 'soldermask_top.gts', 'soldermask_bottom.gbs'],
          drillFile: 'drill.txt',
        };
      default:
  ...
  private async handleBuildSuccess(deploymentId: string, result: DeploymentResult) {
    try {
      ...
      console.log(`Build time: ${formatBuildTime(new Date(Date.now() - buildDuration), new Date())}`);
    } catch (error) {
      console.error(`Failed to handle build success for ${deploymentId}:`, error);
      throw error;
    }
  }
  ...
  private async handleBuildFailure(deploymentId: string, errorMessage: string) {
    try {
      ...
      console.error(`Build failed for deployment ${deploymentId}: ${errorMessage}`);
    } catch (error) {
      console.error(`Failed to handle build failure for ${deploymentId}:`, error);
      throw error;
    }
  }
