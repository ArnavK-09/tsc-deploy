...
export async function getSvgPreviews(
  ...
  for (const svgFile of snapshotResult.svgFiles) {
    try {
      ...
      const type = fileName.includes("-pcb.svg")
        ? "pcb"
        : fileName.includes("-schematic.svg")
          ? "schematic"
          : "3d";

      // Look for corresponding PNG file
      const pngFile = snapshotResult.pngFiles.find(
        (png) => path.basename(png) === fileName.replace(".svg", ".png"),
      );
...
export async function getPngPreviews(
  snapshotResult: SnapshotResult,
): Promise<PngPreview[]> {
  ...
  for (const pngFile of snapshotResult.pngFiles) {
    try {
      ...
      const type = fileName.includes("-pcb.png")
        ? "pcb"
        : fileName.includes("-schematic.png")
          ? "schematic"
          : "3d";

      // Look for corresponding SVG file
      const svgFile = snapshotResult.svgFiles.find(
        (svg) => path.basename(svg) === fileName.replace(".png", ".svg"),
      );
...
export async function snapshotProject(
  ...
  try {
    ...
    for (const circuitFile of circuitFiles) {
      try {
        const { svgFiles, pngFiles } = await generateCircuitSvgs(
          circuitFile,
          snapshotsDir,
          workingDirectory,
        );
        ...
        core.warning(
          `Failed to generate SVGs/PNGs for ${circuitFile}: ${error}`,
        );
    ...
    core.info(
      `✅ Generated ${result.svgFiles.length} SVG snapshot(s) and ${result.pngFiles.length} PNG snapshot(s)`,
    );
    result.success = true;
    result.buildTime = Math.round((Date.now() - startTime) / 1000);
...
async function generateCircuitSvgs(
  circuitFile: string,
  snapshotsDir: string,
  workingDirectory: string,
): Promise<{ svgFiles: string[]; pngFiles: string[] }> {
  ...
  try {
    ...
    try {
      ...
      core.info(
        `📸 Created PCB SVG: ${path.relative(process.cwd(), pcbSvgPath)}`,
      );
      ...
      core.warning(
        `Failed to generate PCB SVG/PNG for ${circuitFile}: ${error}`,
      );
    }
    ...
    try {
      ...
      const schematicSvgPath = path.join(
        snapshotsDir,
        `${baseName}-schematic.svg`,
      );
      const schematicPngPath = path.join(
        snapshotsDir,
      ...
      core.info(
        `📸 Created Schematic SVG: ${path.relative(process.cwd(), schematicSvgPath)}`,
      );

      // Generate PNG from SVG
      await saveSvgAsPng(schematicSvgPath, schematicPngPath, {
        format: "schematic",
      });
      pngFiles.push(schematicPngPath);
    } catch (error) {
      core.warning(
        `Failed to generate Schematic SVG/PNG for ${circuitFile}: ${error}`,
      );
    }
    ...
    try {
      ...
      core.info(
        `📸 Created 3D SVG: ${path.relative(process.cwd(), threeDSvgPath)}`,
      );
      ...
      core.warning(
        `Failed to generate 3D SVG/PNG for ${circuitFile}: ${error}`,
      );
    }
