...
export function generatePreviewUrl(deploymentId: string, domain = 'preview.tscircuit.com'): string {
  return `https://${deploymentId}.${domain}`;
}

export function generatePackageVersion(currentVersion: string, type: 'patch' | 'minor' | 'major' = 'patch'): string {
...
export function extractCircuitFiles(files: Array<{ path: string; content: string }>): Array<{ path: string; content: string }> {
...
export function validateCircuitFile(content: string): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  try {
    if (content.includes('import') && !content.includes('@tscircuit/core')) {
      errors.push('Consider importing from @tscircuit/core for better compatibility');
    }
  } catch (e) {
...
export function parseSemanticCommit(message: string): { type: 'patch' | 'minor' | 'major'; scope?: string } {
